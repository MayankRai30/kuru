...
async def logs(legend):
    if legend.sender_id == OWNER_ID:
        ...
        with open("AltLogs.txt", "w") as logfile:
            logfile.write("⚡ KURUKSHETRA BOTS ⚡ [ Bot Logs ]\n\n" + logs)
        ...
        try:
            await X1.send_file(legend.chat_id, "AltLogs.txt", caption=f"⚡ **KURUKSHETRA BOTS LOGS** ⚡\n  » **ᴛɪᴍᴇ ᴛᴀᴋᴇɴ:** `{ms} ꜱᴇᴄᴏɴᴅꜱ`")
            await fetch.delete()
        except Exception as e:
            await fetch.edit(f"An Exception Occured!\n\n**ERROR:** {str(e)}")

    elif legend.sender_id in SUDO_USERS:
        await legend.reply("» ISKO SIRF MAYANK SIR USE KR SKTE HAI...")
