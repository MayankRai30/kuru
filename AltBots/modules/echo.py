    ...
    if event.sender_id in SUDO_USERS:
        if event.reply_to_msg_id:
            ...
            if user_id in ALTRON:
                await event.reply("BAAP HU TERA.")
            elif user_id == OWNER_ID:
                await event.reply("BETA...MAYANK PAPA PAR ECHO NHI KRTE...")
            elif user_id in SUDO_USERS:
                await event.reply("WO BHI SUDO USER HAI US PAR NHI HOGA....")
            else:
                try:
                ...
                if check in ECHO:
                    await event.reply("» ECHO ACTIVATED HAI ISS PAR !!")
                else:
                    ECHO.append(check)
                    await event.reply("» ECHO ACTIVATED HAI ISS PAR ✅")
        else:
