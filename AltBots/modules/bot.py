...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        ...
        await altron.edit(f"__TUMNE BULAYA MAI CHALA AYA__\n» `{mp} ᴍꜱ`")
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"`SUDO DIYA HAI, AB RUK USKO ADD KR RHA HU....`")
        try:
            await X1.disconnect()
        except Exception:
            pass
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        try:
            ...
            await ok.edit("» REPLY KRKE KARO SIR!!")
            return

        if str(target) in sudousers:
            await ok.edit(f"VO PEHLE SE SUDO USER HAI SIR JI")
        else:
            if len(sudousers) > 0:
        ...
        await event.reply("» SUDO SIRF MAYANK SIR DE SKTE HAI......")
